...
export class PostgresInstitutionalRepository implements InstitutionalRepository {
  ...
  private async createAlert(tenantId: string, data: {
    ...
    // institutional_alerts is a passive UI record — nothing reads it proactively.
    // For severidad alta/crítica, also queue a real email so admins are notified
  ...
  private async notifyTenantAdmins(tenantId: string, alertId: string, title: string, message: string): Promise<void> {
    try {
      ...
      // Best-effort: a notification failure must never block alert creation itself.
    }
  }
