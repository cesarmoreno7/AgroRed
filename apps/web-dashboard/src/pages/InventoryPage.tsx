...
const SOURCE_TYPES = [{ value: "offer_stock", label: "Stock de oferta" }, { value: "rescued_stock", label: "Stock rescatado" }, { value: "buffer_stock", label: "Stock buffer" }];
const SOURCE_COLOR: Record<string, string> = { offer_stock: "#22d3ee", rescued_stock: "#a78bfa", buffer_stock: "#f59e0b" };
const STATUS_COLOR: Record<string, [string, string]> = { available: ["#4ade80","Disponible"], reserved: ["#f59e0b","Reservado"], depleted: ["#f87171","Agotado"], expired: ["#94a3b8","Vencido"] };
...
const empty = { producerId: "", offerId: "", rescueId: "", sourceType: "offer_stock", storageLocationName: "", productName: "", category: "hortaliza", unit: "kg", quantityOnHand: "", quantityReserved: "", municipalityName: "", notes: "", expiresAt: "", latitude: "", longitude: "" };
...
export function InventoryPage() {
  ...
  useEffect(() => { fetchInventory().then(r => { if (r.ok) setItems(r.data); }); }, []);

  const filtered = useMemo(() => {
    const q = search.toLowerCase();
    return items.filter(i => i.productName?.toLowerCase().includes(q) || i.storageLocationName?.toLowerCase().includes(q) || i.municipalityName?.toLowerCase().includes(q) || i.category?.toLowerCase().includes(q));
  }, [items, search]);
  ...
  const handleEdit = (record: any) => {
    ...
    setEditForm({ producerId: record.producerId || "", offerId: record.offerId || "", rescueId: record.rescueId || "", sourceType: record.sourceType || "offer_stock", storageLocationName: record.storageLocationName || "", productName: record.productName || "", category: record.category || "hortaliza", unit: record.unit || "kg", quantityOnHand: String(record.quantityOnHand ?? ""), quantityReserved: String(record.quantityReserved ?? ""), municipalityName: record.municipalityName || "", notes: record.notes || "", expiresAt: record.expiresAt?.slice(0,10) || "", latitude: String(record.latitude ?? ""), longitude: String(record.longitude ?? "") });
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    const res = await updateInventoryItem(editingId, { ...editForm, quantityOnHand: parseFloat(editForm.quantityOnHand), quantityReserved: editForm.quantityReserved ? parseFloat(editForm.quantityReserved) : undefined, latitude: editForm.latitude ? parseFloat(editForm.latitude) : undefined, longitude: editForm.longitude ? parseFloat(editForm.longitude) : undefined, expiresAt: editForm.expiresAt || undefined });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await registerInventoryItem({ ...form, tenantId: user?.tenantId ?? "", offerId: form.offerId || undefined, rescueId: form.rescueId || undefined, quantityOnHand: parseFloat(form.quantityOnHand), quantityReserved: form.quantityReserved ? parseFloat(form.quantityReserved) : undefined, latitude: form.latitude ? parseFloat(form.latitude) : undefined, longitude: form.longitude ? parseFloat(form.longitude) : undefined, expiresAt: form.expiresAt || undefined, notes: form.notes || undefined });
    setLoading(false);
    ...
    <div style={{ color: "#fff", maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: 13, color: "rgba(255,255,255,0.4)" }}>{items.length} ítems · {totalUnits.toLocaleString()} unidades totales</p>
        </div>
        ...
        <div style={{ background: "rgba(34,211,238,0.04)", border: "1px solid rgba(34,211,238,0.15)", borderRadius: 16, padding: 24 }}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 14 }}>
              ...
              <div><label style={lbl}>Tipo de stock *</label><select style={inp} value={form.sourceType} onChange={e => set("sourceType", e.target.value)}>{SOURCE_TYPES.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}</select></div>
              <div style={{ gridColumn: "span 2" }}><label style={lbl}>Ubicación *</label><input style={inp} value={form.storageLocationName} onChange={e => set("storageLocationName", e.target.value)} required /></div>
              ...
              <div><label style={lbl}>Cantidad disponible *</label><input style={inp} type="number" min="0.01" step="0.01" value={form.quantityOnHand} onChange={e => set("quantityOnHand", e.target.value)} required /></div>
              <div><label style={lbl}>Cantidad reservada</label><input style={inp} type="number" min="0" step="0.01" value={form.quantityReserved} onChange={e => set("quantityReserved", e.target.value)} /></div>
              ...
              <div><label style={lbl}>Vencimiento</label><input style={inp} type="date" value={form.expiresAt} onChange={e => set("expiresAt", e.target.value)} /></div>
      ...
      {editingId && (
        <div style={{ background: "rgba(245,158,11,0.04)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 16, padding: 24 }}>
          ...
          <form onSubmit={handleUpdate}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 14 }}>
              <div><label style={lbl}>Tipo de stock</label><select style={inp} value={editForm.sourceType} onChange={e => setE("sourceType", e.target.value)}>{SOURCE_TYPES.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}</select></div>
              <div style={{ gridColumn: "span 2" }}><label style={lbl}>Ubicación</label><input style={inp} value={editForm.storageLocationName} onChange={e => setE("storageLocationName", e.target.value)} /></div>
              ...
              <div><label style={lbl}>Cantidad disponible</label><input style={inp} type="number" step="0.01" value={editForm.quantityOnHand} onChange={e => setE("quantityOnHand", e.target.value)} /></div>
              <div><label style={lbl}>Cantidad reservada</label><input style={inp} type="number" step="0.01" value={editForm.quantityReserved} onChange={e => setE("quantityReserved", e.target.value)} /></div>
              ...
              <div><label style={lbl}>Vencimiento</label><input style={inp} type="date" value={editForm.expiresAt} onChange={e => setE("expiresAt", e.target.value)} /></div>
            ...
            {filtered.map((item, i) => {
              ...
              return (
                <tr key={item.id || i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", background: editingId === item.id ? "rgba(245,158,11,0.04)" : "transparent", transition: "background 0.15s" }}
                  ...
                  <td style={{ padding: "13px 16px", fontSize: 14, fontWeight: 700, color: "#22d3ee" }}>{item.quantityOnHand?.toLocaleString()} <span style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", fontWeight: 400 }}>{item.unit}</span></td>
                  ...
                  <td style={{ padding: "13px 16px", fontSize: 12, color: "rgba(255,255,255,0.4)" }}>{item.expiresAt?.slice?.(0,10) ?? "—"}</td>
                  ...
                  <td style={{ padding: "13px 16px" }}><EditBtn active={editingId === item.id} onClick={() => handleEdit(item)} /></td>
                </tr>
              );
            })}
